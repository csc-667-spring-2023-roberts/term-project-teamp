...
const handleDrawCard = (io, socket, data) => {
  ...
  let index = me.hands.findIndex((c) => {
    if (
      c.color === room.discardcard.color ||
...
const handleCanDiscardCard = (socket, data, callback) => {
  ...
  if (
    room.discardcard &&
    (card.color === room.discardcard.color ||
...
const handleShouldEndTurn = (io, socket, data, callback) => {
  ...
  if (
    room.discardcard.value == -1 &&
    (room.discardcard.type === "pick2" ||
      room.discardcard.type === "skip" ||
      room.discardcard.type === "wild4")
  ) {
    if (room.discardcard.type === "wild4") {
      console.log(room.discardcard.color);
      room.discardcard.color =
        room.discardPile[room.discardPile.length - 2].color;
      console.log(room.discardcard.color);
