...
socket.on("chat", function (event) {
  ...
  document.getElementById("chatbox").innerHTML +=
    '<div class="chattext"><span style="display:flex;"><img src="images/user.png" className="img" style="height: 20px; width:20px"></img>' +
    message.user +
    ": " +
    message.data +
    "</span></div>";
...
socket.on("waitroom-update", function (data) {
  ...
  for (let i = 0; i < room.players.length; i++) {
    players += "<p>" + room.players[i].name + "</p>";
  }
...
socket.on("gameroom-player-update", function (data) {
  ...
  for (let i = 0; i < room.players.length; i++) {
    ...
    if (room.currentplayer === i) {
      cur =
        "&nbsp; <img src='https://cdn-icons-png.flaticon.com/128/1828/1828342.png' height='20px' width='20px'></img> ";
    }
    if (room.players[i].name === user.username) {
      players +=
        '<tr><td style="border:1px solid #fff; border-collapse: collapse;padding:20px">' +
        room.players[i].name +
        " (You) " +
        cur +
        "</td></tr>";
...
socket.on("gameend", function (message) {
  ...
  if (user.username === payload.winner) {
    window.location.href = "/youWin?room=" + payload.room;
  } else {
    window.location.href = "/youLost?room=" + payload.room;
  }
});
